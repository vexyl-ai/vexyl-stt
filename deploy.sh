...
SERVICE_NAME="${SERVICE_NAME:-vexyl-stt}"
REPO_NAME="${REPO_NAME:-vexyl-stt}"
IMAGE="${REGION}-docker.pkg.dev/${PROJECT_ID}/${REPO_NAME}/${SERVICE_NAME}"
HF_TOKEN="${HF_TOKEN:?Set HF_TOKEN env var (HuggingFace gated model access)}"

echo "=== VEXYL-STT → Cloud Run ==="
...
echo "→ Step 3/4: Building image with Cloud Build (this takes ~15-20 min)..."
...
echo "  VEXYL_STT_URL=${SERVICE_URL/https/wss}"
